...
# -*- coding: utf-8 -*-
"""
ЕСКЕРТУ (маңызды): Бір сайттан (koran.kz/trnc/) толық транскрипцияны көшіру — қателер көп;
көрсету үшін эталон емес. Мобильді қолданба аятта араб мәтіні бар кезде транскрипцияны
әрдайым арабтан `arabicTranslitKk` / сервер `quran_translit.py` арқылы шығарады.

Бұл скрипт тек тарихи/қосалқы себептермен `quran.translit` бағанасын толтыру үшін қалдырылған
(мысалы ескі бот немесе экспорт талдауы). Жаңа жоба үшін артықшылық: `text_ar`-тан алгоритммен
толтыру (`fill_kk_translit_gaps.py`, `audit_quran_translit.py`, `validate_quran_translit.py`).

koran.kz ұзын сүрелерде көбінесе араб мәтінін ғана береді; кирилл негізінен қысқа сүрелерде.
Скрипт тек кирилл әріптері бар аяттарды жаңартады.

Дереккөз: https://koran.kz/trnc/
