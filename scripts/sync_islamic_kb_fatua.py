#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fatua.kz мақалаларын islamic_kb.sqlite3 индекстеу."""
...
def main() -> int:
    p = argparse.ArgumentParser(description="Sync Fatua.kz → RAQAT islamic_kb")
    p.add_argument("--url", action="append", dest="urls", help="Бір мақала URL (қайталауға болады)")
    p.add_argument("--max", type=int, default=islamic_kb_sync_max_urls(), help="Sitemap-тен max URL")
    args = p.parse_args()
    ensure_db()
    if args.urls:
        for u in args.urls:
            r = ingest_url(u, source_site="fatua")
            print(f"{r.status}\t{r.url}\tchunks={r.chunks}")
    else:
        stats = sync_site("fatua", max_urls=args.max)
        print(stats)
