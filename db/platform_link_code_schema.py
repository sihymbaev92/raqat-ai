# -*- coding: utf-8 -*-
"""Мобильді ↔ Telegram: 6 таңбалы уақытша байланыс коды."""
from __future__ import annotations

from typing import Any

from db.dialect_sql import is_psycopg_connection, is_sqlite_connection


def ensure_platform_link_code_tables(conn: Any) -> None:
    if is_sqlite_connection(conn):
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS platform_link_codes (
                code_hash TEXT PRIMARY KEY NOT NULL,
                platform_user_id TEXT NOT NULL,
                expires_at TEXT NOT NULL,
                created_at TEXT NOT NULL DEFAULT (datetime('now')),
                FOREIGN KEY (platform_user_id) REFERENCES platform_identities(platform_user_id) ON DELETE CASCADE
            )
            """
        )
        conn.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_platform_link_codes_user
            ON platform_link_codes(platform_user_id)
            """
        )
        return
    if is_psycopg_connection(conn):
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS platform_link_codes (
                code_hash TEXT PRIMARY KEY NOT NULL,
                platform_user_id UUID NOT NULL REFERENCES platform_identities(platform_user_id) ON DELETE CASCADE,
                expires_at TIMESTAMPTZ NOT NULL,
                created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
            )
            """
        )
        conn.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_platform_link_codes_user
            ON platform_link_codes(platform_user_id)
            """
        )
        return
    raise TypeError(f"Unsupported DB connection: {type(conn)!r}")
