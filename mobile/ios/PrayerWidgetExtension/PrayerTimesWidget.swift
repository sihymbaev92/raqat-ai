...
struct PrayerTimesProvider: TimelineProvider {
  ...
  func getSnapshot(in context: Context, completion: @escaping (PrayerTimesEntry) -> Void) {
    completion(PrayerTimesEntry(date: Date(), snapshot: PrayerWidgetSnapshot.parse(from: PrayerWidgetStore.readPayloadJson())))
  }
...
struct PrayerTimesWidgetEntryView: View {
  ...
  var body: some View {
    ZStack {
      LinearGradient(
        colors: [Color(red: 0.05, green: 0.12, blue: 0.16), Color(red: 0.08, green: 0.20, blue: 0.24)],
