...
object PrayerLegacyNotificationCleaner {
  ...
  private val nativeServiceNotificationIds = intArrayOf(904223)
  ...
  /**
   * Ескі азан/Expo хабарламаларын тазалау.
   * @param preserveActiveAzanFsi true болса 904224 FSI (ашу/өшіру) сақталады.
   */
  fun clear(context: Context, preserveActiveAzanFsi: Boolean = false) {
    ...
    try {
      if (preserveActiveAzanFsi) {
        ...
        Log.i(tag, "Cleared legacy prayer notifications (keepFsi=true)")
      } else {
        mgr.cancelAll()
        for (id in legacyAzanNotificationIdStart..legacyAzanNotificationIdEnd) {
          mgr.cancel(id)
  ...
  /** Азан жеткізу кезінде — FSI сақталады, ескілер өшіріледі. */
  fun clearLegacyDuringAzanDelivery(context: Context) {
    clear(context, preserveActiveAzanFsi = true)
  }

  fun clearRepeatedly(context: Context) {
    val app = context.applicationContext
    val preserve = PrayerAzanActiveSession.isActive(app)
    clear(app, preserveActiveAzanFsi = preserve)
    val handler = Handler(Looper.getMainLooper())
    longArrayOf(1_000L, 5_000L, 15_000L).forEach { delayMs ->
      handler.postDelayed(
        {
          val keep = PrayerAzanActiveSession.isActive(app)
          clear(app, preserveActiveAzanFsi = keep)
        },
        delayMs
      )
    }
